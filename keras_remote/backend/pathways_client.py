...
def submit_pathways_job(
    ...
    try:
        ...
        logger.info(
            "View job with: kubectl get %s %s -n %s",
            LWS_PLURAL, job_name, namespace
        )
        return created_lws
        ...
        if e.status == 404:
            ...
            raise RuntimeError(
                f"Kubernetes API error: {e.status} - "
                f"{e.reason}: {e.body}"
            )
        ...
        if pod is not None and pod.status.container_statuses:
            ...
            if container_status.last_state.terminated:
                if container_status.last_state.terminated.exit_code == 0:
                    print(
                        f"[REMOTE] Job {job_name} completed "
...
def _create_lws_spec(
    ...
    pod_template = {
        "metadata": {
            "labels": {"app": "keras-remote-pathways", "job-id": job_id}
        },
        "spec": {
            "containers": [
                {
