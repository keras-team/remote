...
class PoolAddTest(absltest.TestCase):
  def setUp(self):
    ...
    self.mock_load = self.enterContext(
      mock.patch("keras_remote.cli.commands.pool.load_state")
    )
    self.mock_apply = self.enterContext(
      mock.patch(
        "keras_remote.cli.commands.pool.apply_update", return_value=True
      )
    )
...
class PoolAddUpdateFailureTest(absltest.TestCase):
  def setUp(self):
    ...
    self.enterContext(
      mock.patch(
        "keras_remote.cli.commands.pool.load_state",
        return_value=_make_state(),
      )
    )
    self.enterContext(
      mock.patch(
        "keras_remote.cli.commands.pool.apply_update", return_value=False
      )
    )
    self.enterContext(
      mock.patch(
